...
public class FragmentKitchenPlugins extends Fragment implements View.OnClickListener{
    ...
    private DatabaseReference mRef,mRefDishwasher, mRefridge, mRefMicrowave;
    ...
    private void initView(View view) {
        ...
        mRefridge = database.getReference("SmartHome").child(FirebaseAuth.getInstance().getUid()).child("Kitchen")
        ...
        mOpenCoffeeMaker.setOnClickListener(view1 -> {
            if (mOpenCoffeeMaker.isChecked()) {
                ...
                PopUpClass popUpClass = new PopUpClass();
                popUpClass.showPopupWindow(view1, "Fridge is ON");
            } else {

                mRef.child("Fridge").setValue(0).addOnSuccessListener(runnable -> {
                    PopUpClass popUpClass = new PopUpClass();
                    popUpClass.showPopupWindow(view1, "Fridge is OFF");
                });
        ...
        mRefridge.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                // for(DataSnapshot singleSnapshot : dataSnapshot.getChildren()){
                Long user = dataSnapshot.getValue(Long.class);
